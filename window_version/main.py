...
def makeform(root, fields, called_item):
   entries = {}
   root.wm_iconbitmap('imgs/add.ico')
...
def canvas_show_information(image_to_open, text_to_show):
  ...
  pic = ImageTk.PhotoImage(pic.resize((683,420)))
  canvas.create_image((0,28), image = pic, anchor = NW)
  canvas.image = pic
  text_id = canvas.create_text((5, 38),font = ("verdana", 11, "bold"), text = text_to_show, fill = 'green', anchor = 'nw')
  coords = canvas.bbox(text_id)
  xOffset = (683 / 2) - ((coords[2] - coords[0]) / 2)
...
def homepage(canvas, graph_status):
  ...
  photo = ImageTk.PhotoImage(pic.resize((683,420)))
  canvas.create_image((0,0), image = photo, anchor = NW)
  canvas.image = photo
  canvas.create_text((230, 80), font=(_welcome), text = 'WELCOME', fill = 'green', anchor = 'nw')
  canvas.create_text((340, 130), font=(_to), text = 'to', fill = 'green', anchor = 'nw')
  canvas.create_text((110, 175), font=(_import_export), text = 'IMPORT', fill = 'green', anchor = 'nw')
  canvas.create_text((340, 185), font=(_to), text = '&', fill = 'green', anchor = 'nw')
  canvas.create_text((375, 175), font=(_import_export), text = 'Export', fill = 'green', anchor = 'nw')
  canvas.create_text((150, 250), font=(_prediction), text = 'Prediction System', fill = 'green', anchor = 'nw')
  canvas.create_text((270,380), font = ("verdana", 9, "bold"),text = 'Designed By : ANSS Group', fill = 'blue', anchor = 'nw')
  canvas.grid()
  return canvas
...
def about_us(canvas, graph_status):
  ...
  canvas.image = pic
  text_id = canvas.create_text((5, 15),font = ("verdana", 11, "bold"), text = about, fill = 'blue', anchor = 'nw')
  coords = canvas.bbox(text_id)
  xOffset = (683 / 2) - ((coords[2] - coords[0]) / 2)
  canvas.move(text_id, xOffset, 0)
  canvas.grid(columnspan = 4)
...
canvas = Canvas(screen, width = 683, height = 420)
