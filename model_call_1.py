...
def model_calll(window, called_item, model_status, passed_value):
    global called_csv, process_to_do, calling_status, graph_to_print, title_print_first_word
    if called_item == "Rice":
        called_csv = "rice.csv"
    ...
    def graph_print(called_status, graph_to_print, training_data, test_data, len_training):
        ...
        if(graph_to_print == "Line Graph"):
            ...
            plt.scatter(original_data_plot.index.values,
                        original_data_plot['Quantity'], s=10, color='black', label="Training Data")
            plt.scatter(test_data.index.values,
                        test_data['Quantity'], s=10, color='blue', label="Actual Data")
            plt.scatter(predicted_data.index.values,
                        predicted_data['Quantity'], s=10, color='red', label="Predicted Data")
    #            plt.show()
        plt.xlabel("Date")
    ...
    # elif (passed_value =="Reload Model" & model_status == 0):
    #     results_ar = ARIMAResults.load("./model_saved.pkl")
    #     if called_status == 0:
    ...
    #                                 padx=6, pady=3, ipadx=20, ipady=11)
    #     called_status = 1
    #     return called_status


    # else:
    #     graph_print(calling_status, graph_type)
